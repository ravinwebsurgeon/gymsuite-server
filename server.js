...
apiRouter.post('/signin', async (req, res) => {
  ...
  // Generate JWT token
  const token = jwt.sign({ userId: foundUser.userId, email: foundUser.email }, JWT_SECRET, { expiresIn: '1h' });
...
apiRouter.post('/reset-password', async (req, res) => {
  ...
  if (foundUser.resetToken !== resetToken || foundUser.resetTokenExpiry < Date.now()) {
